...
def store(request):
    ...
    emp.save()
    messages.success(request, "nama karyawan sukses di update")
...
def update(request,pk):
    ...
    emp.emp_mobile = request.POST.get('emp_mobile')
    emp.save()
    messages.success(request, "nama karyawan sukses di perbarui")
    return redirect('/')

def deleteEmp(request, pk):
    emp = Emp.objects.get(id = pk)
    emp.delete()
    messages.success(request, "nama karyawan sukses di hapus")
    return redirect('/')
